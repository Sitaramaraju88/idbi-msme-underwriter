...
class ScoringAgent:
    def __init__(self):
        # Checks if GROQ_API_KEY is configured in your environment variables
        self.api_key_configured = os.getenv("GROQ_API_KEY") is not None
        if self.api_key_configured:
            # Using llama3-8b via Groq for sub-second, blazing fast responses
    ...
    def generate_credit_memo(self, s: AgentSignals, score_results: dict) -> str:
        """Generates human-readable narrative context via Groq LLM or fallback interpreter."""
        context_summary = f"""
